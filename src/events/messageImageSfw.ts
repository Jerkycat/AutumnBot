...
export default class ImageMessage implements Event {
    name = 'messageCreate';

    run = async (Client: YumuCore, message: Message) => {
        try {
            if (message.author.bot || message.channel.id !== process.env.SFW_CHANNEL) return;
            ...
            Client.logger.error(`Unexpected error in ImageMessage: ${err}`, 'EventHandler');
        }
    };
    ...
    private async postToTwitter(
        ...
        try {
            const TwitterUser = await Client.accounts.sfw.twitter.currentUserV2();
            if (!TwitterUser) return null;

            const mediaId = await Client.accounts.sfw.twitter.v1.uploadMedia(image, {
                mimeType: 'image/webp'
            });
            const post = await Client.accounts.sfw.twitter.v2.tweet(message, {
    ...
    private async postToBluesky(
        ...
        try {
            ...
            const BlueskyPost = await Client.accounts.sfw.bluesky.post({
                text: message,
                images: [{ data: blob }],
                langs: ['pt', 'br', 'en']
            });
            ...
            if (BlueskyPost) {
                ...
                const url = `https://bsky.app/profile/${Client.accounts.sfw.bluesky.profile.handle}/post/${postId}`;
